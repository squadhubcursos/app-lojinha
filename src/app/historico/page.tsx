...
export default function HistoricoPage() {
  ...
  function TabelaCompras({ lista }: { lista: Compra[] }) {
    if (lista.length === 0) {
      return <p className="text-center text-gray-400 py-8">Nenhuma compra neste período.</p>
    }
    ...
    return (
      <div>
        <div className="bg-[#009ada]/10 rounded-xl p-4 mb-4 flex items-center justify-between">
          <span className="text-gray-600 font-medium">Total do período</span>
            ...
            <tbody>
              {lista.map((compra) => (
                <tr key={compra.id} className="border-b hover:bg-gray-50">
                  <td className="py-2 px-3 text-gray-600">{formatDate(compra.comprado_em)}</td>
        ...
        {loading ? (
          ...
          <Tabs defaultValue="semana">
            <TabsList className="mb-4 w-full">
              ...
              <TabsTrigger value="mes_atual" className="flex-1">Este mês</TabsTrigger>
              <TabsTrigger value="mes_anterior" className="flex-1">Mês anterior</TabsTrigger>
            </TabsList>
